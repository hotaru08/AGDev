...
void MapEditor::renderObj(void)
{
	if (mapEditing)
	{
		...
		case TURRET:
		{
			...
			modelStack.Rotate(rotate, 0.f, 1.f, 0.f);
			modelStack.Scale(scale.x, scale.y, scale.z);
			RenderHelper::RenderMeshWithLight(MeshBuilder::GetInstance()->GetMesh("Turret_Top"));
			modelStack.PopMatrix();
			...
			modelStack.Rotate(rotate, 0.f, 1.f, 0.f);
			modelStack.Scale(scale.x, scale.y, scale.z);
			RenderHelper::RenderMeshWithLight(MeshBuilder::GetInstance()->GetMesh("Turret_Bottom"));
			modelStack.PopMatrix();
			break;
		}
		}
...
void MapEditor::updateOption(double dt)
{
	...
	if (KeyboardController::GetInstance()->IsKeyPressed(VK_NUMPAD5))
	{
		...
		case TURRET:
		{
			GenericEntity* Turret = Create::Entity("Turret_Top", displacement, scale);
			Turret->SetRotate(rotate);
			Turret->InitLOD("Turret_Top", "Turret_Top_Mid", "Turret_Top_Lo");

			GenericEntity* Turret2 = Create::Entity("Turret_Bottom", displacement, scale);
			Turret2->SetRotate(rotate);
			Turret2->InitLOD("Turret_Bottom", "Turret_Bottom_Mid", "Turret_Bottom_Lo");
			break;
		}
		}
	}
