...
void CSpatialPartition::Update(void)
{
	...
	for (std::list<EntityBase*>::iterator it = EntityManager::GetInstance()->returnEnemy().begin(); it != EntityManager::GetInstance()->returnEnemy().end(); ++it)
	{
		EntityBase* test = *it;
		egridX = ((test->GetPosition().x >= 0) ? (
			int)((test->GetPosition().x / xGridSize)) :
			(int)(((test->GetPosition().x - xGridSize) / xGridSize))) * xGridSize + (xGridSize / 2);

		egridZ = ((test->GetPosition().z >= 0) ?
			(int)((test->GetPosition().z / zGridSize)) :
			(int)(((test->GetPosition().z - zGridSize) / zGridSize))) * zGridSize + (zGridSize / 2);
	}

	//cout << GetGrid(gridX, gridZ).GetListOfObject().size() << endl;
...
void CSpatialPartition::Render(Vector3* theCameraPosition)
{
	...
	//	modelStack.PushMatrix();
	//	modelStack.Translate(egridX, yOffset, egridZ);
	//	modelStack.Scale(xGridSize, 0.1, zGridSize);
	//	//modelStack.Rotate(90, 1, 0, 0);
	//	RenderHelper::RenderMesh(MeshBuilder::GetInstance()->GetMesh("gridpos"));
	//	modelStack.PopMatrix();
...
float CSpatialPartition::GetObjectsSize()
{
	// Get the indices of the object's position
	int xIndex = (((int)CPlayerInfo::GetInstance()->GetPos().x - (-xSize >> 1)) / (xSize / xNumOfGrid));
	int zIndex = (((int)CPlayerInfo::GetInstance()->GetPos().z - (-zSize >> 1)) / (zSize / zNumOfGrid));

	return theGrid[xIndex*zNumOfGrid + zIndex].GetListOfObject().size();
}
