...
void GenericEntity::Render()
{
	...
	if (GetLODStatus()) // LOD is true
	{
		...
		if (theDetailLevel != NO_DETAILS)
			RenderHelper::RenderMesh(GetLODMesh()); // render the mesh of the detail
	}
	else
		RenderHelper::RenderMesh(modelMesh); // LOD is false
								...
								const Vector3& _scale)
{
	...
	result->SetAABB(Vector3(_scale.x, _scale.y * 2, _scale.z), -Vector3(_scale.x, 0, _scale.z));
	...
	const Vector3& _scale)
{
	...
	result->SetPosition(_position);
	result->SetScale(_scale);
	result->SetAABB(Vector3(_scale.x, _scale.y, _scale.z), -Vector3(_scale.x, _scale.y, _scale.z));
	result->SetCollider(true);
	return result;
}