...
void CGrid::Add(EntityBase* theObject)
{
	...
	{
		// If objects already inside the current grids
		if (ListOfObjects[i] == theObject)
			return;
	}
	// not in list, add into list
...
{
	std::vector<EntityBase*> ListOfActive;

	for (std::vector<EntityBase*>::iterator it = ListOfObjects.begin(); it != ListOfObjects.end(); ++it)
	{
		EntityBase* temp = *it;
		if (temp->IsDone() == false)
		{
			ListOfActive.push_back(temp);
		}
	}
